...
CREATE TABLE `c4game` (
  ...
  PRIMARY KEY (`gameId`),
  CONSTRAINT fk_player1 FOREIGN KEY (player1)
	REFERENCES c4users(Name),
	 CONSTRAINT fk_player2 FOREIGN KEY (player2)
	REFERENCES c4users(Name)
) ;
...
CREATE TABLE `c4chat` (
  ...
  `gameId` int(11) DEFAULT NULL,
  PRIMARY KEY (`id`),
  CONSTRAINT fk_chat_player FOREIGN KEY (userId)
	REFERENCES c4users(Name),
   CONSTRAINT fk_chat_game FOREIGN KEY (gameId)
	REFERENCES c4game(gameId)
)  ;
